...
import { Entry, Prisma, PrismaPromise } from "@prisma/client";
...
interface EntriesService {
    ...
    createEntry(createEntryRequest: CreateEntryRequest, userId: number): Promise<EntryDto>;
    updateEntry(id: number, updateEntryRequest: UpdateEntryRequest): Promise<EntryDto>;
...
export default class EntryServiceImpl implements EntriesService {
    ...
    async updateEntry(id: number, updateEntryRequest: UpdateEntryRequest): Promise<EntryDto> {
        const { name, description, emotions, userSecret } = updateEntryRequest;
        return new EntryDto(await this.prisma.entry.update({
            where: {id: id},
            data: {
                name: name,
                description: this.encryptionService.encrypt(description, userSecret),
